...
applications due to its high performance and it has included facilities to support
parallel computing in the standard library, such as \textit{thread} object,
\textit{mutex}, \textit{lock} and etc.  The standard library enables the programmers 
to implement the parallel applications using those low-level APIs, but it is 
not scalable when the 
